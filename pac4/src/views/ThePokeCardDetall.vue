...
let infoJson = JSON.parse(props.elpokemon);
onMounted(()=>{
...
<template >
  <div class="contenidor_detalls">
    <div class="card-detall" :id="infoJson.id">
      <div class="cont-detall">
        <section class="nombre">
          <div>
            <p class="pk_id">{{ infoJson.id }}</p>
            <p class="pk_name">{{ infoJson.name }}</p>
          </div>
        </section>
        <section class="imagenes">
          <img class="pk_img" :src="infoJson.imageFront" />
          <img class="pk_img2" :src="infoJson.imageBack" />
        </section>
        <section class="tipus">
          <tiposPoke v-for="tipo in infoJson.tipus" :key="tipo" :tipo="tipo" />
        </section>
        <section class="stats">
          <statsPoke v-for="stat in infoJson.stats" :key="stat" :stat="stat" />
        </section>
        <section class="utilitats">
          <div class="opcions-d">
            <a id="tancar-detall" href="">
              <router-link :to="{ name: 'pokedex', params: { npk: 10, dl: true }}">

              <img
                src="../assets/img/icons8-cerrar-ventana.gif"
                alt="detalls"
              />
              <span class="tooltips">Tancar detalls</span>
              </router-link>
            </a>
          </div>
        </section>
